    ...
    public override async chatInputRun(interaction: ChatInputCommandInteraction): Promise<void> {
        ...
        if (existingRole) {
            await interactionManager.edit(t(
                'commands:addcountry.error.alreadyExists',
        ...
        if (guild.roles.cache.size > 249) {
            await interactionManager.edit(t('commands:addcountry.error.noSlotLeft', { emoji: '❌' }));
        ...
        try {
            ...
            error(
                guild.id,
                `Could not create country role ${frenchName}: ${err}`,
                this.logFooter
            );

            this.container.logger.error(err);
            await interactionManager.edit(
                t('commands:addcountry.error.couldNotCreateRole', { emoji: '❌' })
            );

            return;
        }

        await this.container.prisma.country.create({
            data: {
        ...
        await interactionManager.edit(t('commands:addcountry.confirm', {
