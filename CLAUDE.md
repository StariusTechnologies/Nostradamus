...
Beyond those flat top-level stores, self-contained features can live under **`src/modules/<name>/`**. A module folder mirrors the store layout internally — any of `commands/`, `listeners/`, `interaction-handlers/`, `preconditions/` — plus non-piece files at the module root (services, data, word lists) that its own pieces import. `Bootstrap.registerModules()` (called from `index.ts` after `initializeClient()` and **before** `login()`, since login is when stores scan their paths) walks each `src/modules/<name>/<store>/` and registers it with the matching Sapphire store, so those pieces load alongside the flat ones; it logs `Registered N module(s): …` at boot. Only the piece-store subfolders are scanned — **module-root files are never picked up as pieces**, which is exactly where a module's service singleton or constant data belongs. Pieces inside a module follow the identical conventions to flat ones (PascalCase filename, `@ApplyOptions` + default-export class). `src/modules/ansy-detection/` is the reference example: a `messageCreate` listener plus an `AnsyDetection` service and `Words.ts` data at the root, DMing the owner (`process.env.OWNER`) when someone talks about them without pinging. Note: dibog's `make_module` scaffolds this tree but names generated piece files kebab-case — rename (or hand-write) them PascalCase to match the repo before committing.
...
Settings access goes through `src/lib/SettingsService.ts`:

- `SettingKey` enum — extend this when adding any new per-guild setting; the `Setup` command iterates this enum to decide which slash-command options to expose.
- `getSetting(guildId, key)` / `saveSetting(guildId, key, value)` / `removeSetting(guildId, key)` — always go through these helpers, never raw `prisma.settings.*`. That rule is what makes the cache below correct: the service is the only writer, so nothing can invalidate behind its back.
- **Reads are served from an in-memory cache.** `warmSettingsCache()` runs first in `Ready.ts`, before `WatchService.init()` and `InviteCache.init()`, and loads every row in one `findMany`. `getSetting` falls back to a single `findUnique` on a miss (a new guild, or a key that has never been set) and caches the result. Misses are cached as `null` via `Map.has` semantics rather than `?? fetch`, so an unset optional key like `AttachmentStorageChannel` does not round-trip on every read. `saveSetting` and `removeSetting` both update the cache in the same call.
- The cache holds the **raw string**, never the formatted value, so adding a formatter later does not change what is stored. It assumes a **single process**: if V5 is ever sharded, a `/setup` on one shard will not invalidate the others, and the staleness is silent.
